...
def analyze_session(boat1_path: str, boat2_path: str = None) -> list[dict]:
    ...
    if boat2_path is None:
        ...
        intervals = detect_maneuvers_from_sog_minima(boat1_df)
...
"""def detect_maneuvers_from_sog_minima(
    ...
    # Smooth SOG if not already done
    if 'SOG_smoothed' not in boat_df.columns:
        boat_df['SOG_smoothed'] = boat_df['SOG'].rolling(window=90, center=True, min_periods=1).mean()
    ...
    # minima_idx = argrelextrema(sog, np.less, order=1)[0] #5 is good
    minima_idx = argrelmin(sog, order=1)[0] #5 is good
    # Sort chronologically
    maxima_idx = sorted(maxima_idx)
    minima_idx = sorted(minima_idx)
    print(minima_idx)
...
def detect_maneuvers_from_sog_minima(
    ...
    minima_idx, props = find_peaks(inverted, prominence=prominence, width=width)

    maneuvers = []
    for idx in minima_idx:
        i1 = int(props['left_ips'][np.where(minima_idx == idx)][0])
        i2 = int(props['right_ips'][np.where(minima_idx == idx)][0])
...
def detect_maneuvers_from_sog_minima(
    ...
    for min_idx in minima_idx:
        ...
        i1 = left_max_candidates[-1]
        i2 = right_max_candidates[0]
